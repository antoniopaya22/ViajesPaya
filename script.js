...
function countryPage(country) {
  ...
  const foodSection = food ? `<section class="section guide-section"><div class="shell"><div class="section-heading"><div><span class="section-kicker">COMIDA TÍPICA</span><h2>${food.title}</h2></div><p>${food.intro}</p></div>${foodToc}<div class="guide-body">${renderBlocks(food.blocks)}</div></div></section>` : '';
